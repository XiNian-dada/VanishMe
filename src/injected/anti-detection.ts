...
export function installAntiDetection(): void {
  ...
  try {
    ...
    if (navLangDesc) nativeDescriptors.set('Navigator.prototype.languages', navLangDesc);
    if (navLangDescSingle) nativeDescriptors.set('Navigator.prototype.language', navLangDescSingle);
    if (dateOffsetDesc) nativeDescriptors.set('Date.prototype.getTimezoneOffset', dateOffsetDesc);
  } catch (e) {
  ...
  try {
    Object.getOwnPropertyDescriptor = function(obj: any, prop: string | symbol): PropertyDescriptor | undefined {
      ...
      if (obj === Navigator.prototype && prop === 'languages') {
        ...
        if (saved) return { ...saved }; // Return a copy of the original
      }
